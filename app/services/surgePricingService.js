import * as repo from "../repositories/surgePricingRepository.js";
import { AppError } from "../utils/AppError.js";
import { transformSurge, transformSurges } from "../transformers/surgeTransformer.js";

export const getSurgePricings = async (filters) => {
    const result = await repo.findAll(filters);
    return {
        success: true,
        message: "Surge pricings fetched successfully.",
        data: transformSurges(result.rows),
        pagination: {
            page:       result.page,
            limit:      result.limit,
            total:      result.total,
...
export const getSurgePricingById = async (id) => {
    ...
    return {
        ...
        data: transformSurge(record),
    };
};

export const getActiveSurgeByTime = async (city_id, vehicle_type_id, time) => {
    const record = await repo.findActiveByTime(city_id, vehicle_type_id, time);
    return {
        success: true,
...
export const deleteSurgePricing = async (id) => {
    ...
    return { success: true, message: "Surge pricing deleted successfully." };
};